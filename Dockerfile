...
RUN apt-get install -y git python3 wget python3.10-venv

COPY src/ /opt/workdir/src
COPY Makefile /opt/workdir
...
RUN apt-get update
RUN apt-get install -y bash zsh fish git python3
RUN apt-get install -y cloc openjdk-11-jdk-headless git python3
...
# COPY src/commands/maat.sh /opt/workdir/bin/maat
# COPY cache/code-maat-1.0.4-standalone.jar /opt/workdir/bin/code-maat.jar

# What the container should run when it is started.
# ENTRYPOINT [ "/bin/hello.sh" ]
# ENTRYPOINT [ "/bin/bash" ]
ENV PATH="/opt/workdir/bin:${PATH}"
